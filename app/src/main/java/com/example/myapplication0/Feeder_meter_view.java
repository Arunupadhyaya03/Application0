...
public class Feeder_meter_view extends Fragment {
    TableLayout table;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        String res="";
        try {
            res=new BackgroundWorker().execute("fmmview").get();
            Log.v(TAG,"res      .."+res);
            JSONObject feedermeterv = new JSONObject(res);
            JSONArray feedermeterarray=feedermeterv.getJSONArray("result");
            if(feedermeterarray.length()!=0){
             ...
             //   row_head.addView(tb13_head);
                ...
                for (int i = 0; i < feedermeterarray.length(); i++){
                    JSONObject feedermeter = feedermeterarray.getJSONObject(i);
                    // String Meter=(subdivdata.getString("Meter"));
                    String sl_no=(feedermeter.getString("sl_no"));
                    String feeder_no = (feedermeter.getString("feeder_no"));
                    String meter_no=(feedermeter.getString("meter_no"));
                    String meter_feature = (feedermeter.getString("meter_feature"));
                    String conn_type = (feedermeter.getString("conn_type"));
                    String conn_name = (feedermeter.getString("conn_name"));
                    ...
                    tb3.setText(String.format("%.2f",Double.parseDouble(meter_no)));
                    tb4.setText(meter_feature);
